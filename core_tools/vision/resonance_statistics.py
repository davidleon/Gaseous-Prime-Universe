...
class ResonanceStatistics:
    ...
    def scale_invariance_test(self, max_N=100000, steps=5):
        ...
        """
        print(f"LOG: Analyzing Scaling Invariance (N up to {max_N})")
        print(f"{'Limit':<12} | {'P_echo':<10} | {'Variance'}")
        print("-" * 45)
        ...
        print(f"Mean Scaling Stability: {avg_stability:.4f}")
    ...
    def shockwave_correlation(self, window_size=100, limit=10000):
        ...
        print(f"LOG: Variance-to-Mean Ratio (VMR): {vmr:.4f}")
        return vmr
    ...
    def confidence_interval_analysis(self, limit=10000):
        ...
        print(f"LOG: Asymptotic Confidence Level: {confidence * 100:.6f}%")
        return confidence
